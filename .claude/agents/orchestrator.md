...
**Proof-of-work, not self-report (binding — CLAUDE.md §3 / ADR-011):** evidence (a)+(b) is satisfied ONLY by a captured artifact, never by a typed "BOOT_OK ✓". To produce it you dispatch `coder` (Bash + emulator) to run **`bash tool/run_smoke.sh <phase_id>`** — it builds with `--dart-define=GIT_SHA=$(git rev-parse --short HEAD)`, boots on a real device, runs the non-mocked e2e, and writes `.project/qa-runs/smoke-<phase>-<sha>-<ts>.log` (with `BOOT_OK …sha=…` + `FIRST_SCREEN_OK` + `SMOKE_RESULT exit=0 sha=…`). `coder` references that log path in `## Integration Smoke`. You (no Bash) then `Read` the log to confirm it exists and shows `SMOKE_RESULT exit=0` with the current short SHA. The **`verify-smoke.py` hook is the mechanical backstop** — it blocks the stop if the artifact is missing/stale/sha-mismatched/marker-incomplete, so you cannot advance on a fabricated claim even if you tried.

If any is absent or partial, route back to `coder` (or `app-bootstrap` on phase 01) and have it RUN `tool/run_smoke.sh` — do NOT advance, and do NOT accept a hand-written evidence block. If the smoke FAILS, set status back to `IN_PROGRESS` (bug loop). **In autonomous mode, after 2 consecutive smoke failures on the same phase, HALT and surface to the user** (never build on a broken foundation, never churn tokens indefinitely). This gate is NEVER skippable: not conditionally, not via `## Skipped Steps`, and **not in autonomous mode** (`auto_approve: true` bypasses human-approval gates, NOT runtime verification). Green tests + clean analyze + passing reviews are not a running app — a phase never built/booted/run-against-real-backend has NOT passed this gate.
