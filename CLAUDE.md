...
- The CI runtime jobs (`build-and-boot`, `build-ios`, `backend-integration`, `integration-smoke`) MUST be green before a phase leaves `INTEGRATION_SMOKE`. Static green (analyze + mocked tests + line coverage) is necessary but NOT sufficient — it is not a running app. These jobs run the **same `tool/run_smoke.sh`** the in-loop gate uses; locally the `verify-smoke.py` hook is the mechanical proof-of-work backstop (§3, ADR-011) — evidence is a captured artifact, never a typed claim.
