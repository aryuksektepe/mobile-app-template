...
**Evidence is PROOF-OF-WORK, not self-report (binding — ADR-011):** the above criteria are NOT satisfied by the agent typing "BOOT_OK ✓ / e2e passed ✓". They are satisfied ONLY by a captured artifact from an actually-executed run. Every phase at INTEGRATION_SMOKE, the orchestrator dispatches `coder` (has Bash + emulator) to run **`tool/run_smoke.sh <phase_id>`** — the single source of truth (CI calls the same script). It builds with `--dart-define=GIT_SHA=$(git rev-parse --short HEAD)`, boots on a real device, runs the non-mocked e2e, and writes `.project/qa-runs/smoke-<phase>-<sha>-<ts>.log` containing the app-emitted `BOOT_OK …sha=…` + `FIRST_SCREEN_OK` + a final `SMOKE_RESULT exit=0 sha=…`. The phase archive's `## Integration Smoke` references that log path. The **`verify-smoke.py` hook mechanically blocks** the transition out of INTEGRATION_SMOKE unless the artifact exists, its sha == HEAD, it is fresher than `lib/`, and all three markers are present. This runs IN-LOOP every phase (not deferred to manual testing) — so bugs surface localized at phase N, not compounded across 20 phases. In auto mode, after **2 consecutive smoke failures on the same phase, HALT and surface to the user** (no infinite churn / no building on a broken foundation). Skill: `flutter-build-boot-gate`.
...
- The gate's authoritative evidence is the **local `tool/run_smoke.sh` artifact** (`.project/qa-runs/smoke-*.log`), enforced every phase by the `verify-smoke.py` hook (§3, ADR-011) — this needs NO GitHub minutes and is the binding check. Static green (analyze + mocked tests + line coverage) is necessary but NOT sufficient — it is not a running app. The GitHub CI runtime jobs (`build-and-boot`, `build-ios`, `backend-integration`, `integration-smoke`) run that same `run_smoke.sh` in the cloud, but are **manual-only (`workflow_dispatch`)** by default (ADR-013) — an OPTIONAL on-demand batch confirmation (e.g. pre-release), not a per-phase requirement. A project that wants cloud CI to gate every phase can re-enable `push:`/`pull_request:` triggers (and budget the minutes).
